...
try{
 let claims = await Promise.all(
    ...
    claimsData.map((claims,index) => {let processed = processClaims(claims);
        claimsReturn[CLAIMSGRAPH[index].NETWORK] = processed});
   return claimsReturn
} catch(error)
{console.log("could not fetch claims");return null
}}